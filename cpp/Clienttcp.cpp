...
Clienttcp_i::Clienttcp_i(const char *uuid, const char *label) :
    Clienttcp_base(uuid, label)
{
    // Avoid placing constructor code here. Instead, use the "constructor" function.

}

Clienttcp_i::~Clienttcp_i()
{
}

void Clienttcp_i::constructor()
{
    /***********************************************************************************
     This is the RH constructor. All properties are properly initialized before this function is called 
    ***********************************************************************************/
}
void Clienttcp_i::start() throw (CF::Resource::StartError, CORBA::SystemException)
{
	Clienttcp();
}

int Clienttcp_i::Clienttcp(void)
{
	int sock =0, valread;
	struct sockaddr_in serv_addr;
	char *hello = "Hello msg from client";
	char buffer[1024] = {0};
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void Clienttcp_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &Clienttcp_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &Clienttcp_i::<callback method>)
        in the constructor.
        ...
        //Add to Clienttcp.cpp
        Clienttcp_i::Clienttcp_i(const char *uuid, const char *label) :
            Clienttcp_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &Clienttcp_i::scaleChanged);
            addPropertyListener(status, this, &Clienttcp_i::statusChanged);
        }

        void Clienttcp_i::scaleChanged(float oldValue, float newValue)
        {
            RH_DEBUG(this->_baseLog, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void Clienttcp_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
...
************************************************************************************************/
int Clienttcp_i::serviceFunction()
{
